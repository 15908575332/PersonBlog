...
var Paul_Pio = function (prop) {
  ...
  var modules = {
    ...
    showByDirective: function (element) {
      // 获取元素属性值
      const getAttr = name => element.getAttribute(name);

      // 提取提示文本，优先使用自定义属性，其次使用元素文本
      const text = getAttr('data-pio-text') || getAttr('text') || element.innerText.trim();

      // 提取提示类型，默认为'default'
      const type = getAttr('data-pio-type') || getAttr('type') || 'default';
      ...
      if (text) {
        ...
        switch (type) {
          case 'read':
            finalText = `想阅读 「${text}」 这篇文章吗？`;
            break;
          case 'link':
            finalText = `想前往 「${text}」 这个链接吗？`;
            break;
          case 'contact':
            finalText = `想联系我 「${text}」 吗？`;
            break;
          case 'music':
            finalText = `想听听 「${text}」 这首音乐吗？`;
            break;
          case 'look':
            finalText = `想查看 「${text}」 吗？`;
            break;
          case 'album':
            finalText = `想查看 「${text}」 这个相册吗？`;
            break;
          case 'custom':
            finalText = `${text}`;
            break;
  ...
  var action = {
    ...
    buttons: function () {
      ...
      elements.home.onclick = function () {
        ...
        let port = prop.content.homePort ? ':' + prop.content.homePort : '';
          ...
          for (var j = 0; j < e.length; j++) {
            if (t.type === 'read') {
              e[j].onmouseover = function () {
                ...
                modules.render(`想阅读 「${text}」 这篇文章吗？`);
              };
            } else if (t.type === 'link') {
              e[j].onmouseover = function () {
                const text = (this.dataset && this.dataset.tip) || this.innerText || '';
                modules.render(`想前往 「${text}」 这个链接吗？`);
              };
            } else if (t.type === 'chart') {
              e[j].onmouseover = function () {
                const text = (this.dataset && this.dataset.tip) || this.innerText || '';
                modules.render(`想联系我 「${text}」 吗？`);
              };
            } else if (t.type == 'music') {
              e[j].onmouseover = function () {
                const text = (this.dataset && this.dataset.tip) || this.innerText || '';
                modules.render(`想听听 「${text}」 这首音乐吗？`);
              };
            } else if (t.type == 'look') {
              e[j].onmouseover = function () {
                const text = (this.dataset && this.dataset.tip) || this.innerText || '';
                modules.render(`想查看 「${text}」 吗？`);
              };
            } else if (t.type == 'album') {
              e[j].onmouseover = function () {
                const text = (this.dataset && this.dataset.tip) || this.innerText || '';
                modules.render(`想查看 「${text}」 这个相册吗？`);
              };
  ...
  this.init = function (onlyText) {
    if (!(prop.hidden && modules.isMobile())) {
      // if (!(!prop.hidden && !modules.isMobile())) { // 移动端也展示看板娘
      if (!onlyText) {
        action.welcome();
