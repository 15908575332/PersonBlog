...
const initConfig = {
  ...
  content: {
    ...
    homeLink: 'http://snows-l.site', // 首页链接
...
function onModelLoad(model) {
  ...
  function playAction(action) {
    action.text && pio_instance.modules.render(action.text);
    action.motion && pio_instance.model.motion(action.motion);
  ...
  canvas.onclick = function () {
    ...
    const action = pio_instance.modules.rand(touchList);
    playAction(action);
  };
